...
internal object InMemoryPersistenceDelegate : PersistenceDelegate {
    ...
    override suspend fun createPlayer(credentials: Credentials): Player {
        ...
        return playerMapStore.observe().first().getPlayerWithUsername(credentials.username)
    }

    override suspend fun getPlayerById(id: Int): Player =
        playerMapStore.observe().first()[id]?.player ?: throw NoPlayerWithIdException(id)

    // TODO Create a distance based filter
    override suspend fun getPlayersNear(
        location: Location,
        distanceFilter: DistanceFilter
    ): List<Player> = playerMapStore.observe().first().values.map(PlayerWithCredentials::player)
